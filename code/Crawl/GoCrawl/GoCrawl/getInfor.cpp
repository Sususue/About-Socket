...
void getContent(const string& recvStr, ClassInfor* ACourse) {
	...
	*/
	...
	if (index != -1) {
		...
		if (temp != -1) {//outLineStructureDtos:后面有内容，此处为json
			str = regex_replace(str, regex("[\f\r\t\n]"), "");
...
/*寻找网页url*/
void findurl(string recvStr, Myqueue* schoolUrl, Myqueue* school) {
	int index, index1, indexEnd;//前两个用于搜寻url，最后一个用于确定边界
	string url,name;
	index = recvStr.find("<div class=\"u-usitys f-cb\">");
	int cnt = 8;//先不跑完，人为控制只跑cnt个学校
	if (index != -1) {
		indexEnd = recvStr.find("</div>", index);//即在index和indexend之间都是学校url信息
		while (index < indexEnd && cnt > 0) {
			index = recvStr.find("href=\"",index);
			index += strlen("href=\"");
			index1= recvStr.find("\"", index);
			url = recvStr.substr(index, index1 - index);
			name=findSchoolName(url);
			(*schoolUrl).pushQ(url);//进队
			(*school).pushQ(name);
			index = index1;
			cnt--;
		}
...
/*寻找学校id*/
string findid(string recvStr) {

	string id;
	int index = recvStr.find("window.schoolId = \"");
	if (index != -1) {
		index += strlen("window.schoolid = \"");
		int index1 = recvStr.find("\"", index);
		id = recvStr.substr(index, index1 - index);
		return id;
	}
	else {
		return "";
...
/*找到学校简称*/
string findSchoolName(string recvStr) {
	string name;
	int index = recvStr.find("/university/");
	if (index != -1) {
		index += strlen("/university/");
		name = recvStr.substr(index, strlen(recvStr.c_str()) - index);
		return name;
	}
	else {
		return "";
